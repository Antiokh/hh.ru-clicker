...
def test_200_empty_missing_ok_and_query_params(monkeypatch):
    ...
    assert out == {"ok": True, "resume_id": "rh1", "hard_missing": [], "soft_missing": []}
    q = _query()
...
def test_200_list_of_strings_missing(monkeypatch):
    ...
    assert out == {"ok": False, "resume_id": "rh1", "hard_missing": ["WORK_FORMAT"], "soft_missing": ["PHOTO"]}
...
def test_200_list_of_dicts_type_or_id(monkeypatch):
    ...
    assert out == {"ok": True, "resume_id": "rh1", "hard_missing": [], "soft_missing": ["PHOTO", "EDUCATION"]}
...
def test_200_not_a_list_defensively_empty(monkeypatch):
    ...
    assert out == {"ok": False, "missing": [], "reason": "invalid_response"}
